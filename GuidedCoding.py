...
from ausgabe.ausgabe_anzeigen1 import ausgabe
...
# ------------------------------------------------------------------------------------------------------------
    ...
    def ausgabe_aufruf(self, code):
        ...
        if code in ["Hier den Code eingeben...\n", "\n"]:
            return
...
# -----------------------------------------------------------
        ...
        button_f.columnconfigure(1, weight = 0)
        button_f.columnconfigure(2, weight = 0)



# -----------------------------------------------------------
        # BUTTON zum ausführen des Codes
        ausführen_b = tb.Button(button_f, text = "ausführen", command = lambda: self.ausgabe_aufruf(self.eingabe_code_t.get(1.0, "end")))
        ausführen_b.grid(row = 0, column= 2, pady = 10, sticky= "e")
...
# -----------------------------------------------------------
        ...
        self.prüfungsmodus = tb.Text(button_f, font = ("Helvetica", 12), height=1, width=23)
        self.prüfungsmodus.insert(1.0, "Prüfungsmodus")
        self.prüfungsmodus.config(background="#6F0000", highlightbackground="#6F0000", highlightcolor="#6F0000")
...
# -----------------------------------------------------------------------------------
        ...
        self.ausgabe_ki_t.insert(1.0, "Erklärung der KI") # für den Anfang wird Ausgabe Angezeigt
...
# -----------------------------------------------------------
        # MENUBAR
        menu = menubar(self.root, self.eingabe_code_t, self.prüfungsmodus)
        globals.menubar = menu
