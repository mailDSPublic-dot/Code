...
class AI():
    ...
    def fehler_fuer_ki(self, fehler_typ, code, code_zeile):
        # Gibt einen formatierten Prompt zurück
        return (f"Fehlertyp: {fehler_typ}\n"
            f"Code: {code}\n"
            f"Codezeile: {code_zeile}\n\n"
            f"Du bist ein Lehrer, sage keine Lösung vor, sondern erkläre, was der Fehler ist. Nutze keine Markierungen. Antworte auf deutsch. Fasse dich kurz.")
    ...
    def chatbot(self, code: str, fehler: Exception):
        ...
        if isinstance(fehler, SyntaxError):
            ...
            prompt = self.fehler_fuer_ki(fehler_typ, code, code_zeile)
